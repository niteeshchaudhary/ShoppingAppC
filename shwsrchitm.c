...
void load_images()
{
	...
	hli=(HBITMAP)LoadImageW(g_hinst,L"./resources/searchresult.bmp",IMAGE_BITMAP,width,height,LR_LOADFROMFILE);
...
/*	HMENU hMenu = CreateMenu();
	...
	insert_value(&cid[SEARCH],474, 20, 495, 50,1);
	wchar_t wtosch[15];
    ...
    for(int ci=0;ci<7;ci++)
	{
		for(int j=0;j<categ[ci].itm_no;j++){
			if(match(tosch,categ[ci].itm[j].title,70,70)){
				printf("\n{ %s num=%d }",categ[ci].itm[j].title,j*10+ci);
				 HWND ws = CreateWindow("cslist", NULL,
					...
					itm_num++;
					SendMessageW(ws, SETINFO, j*10+ci, (LPARAM)&categ[ci].itm[j]);	
				found++;
			}
		}
	}	             
